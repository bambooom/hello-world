...
pub async fn echoes(first: TcpListener, second: TcpListener) -> Result<(), anyhow::Error> {
    let handle1 = tokio::spawn(echo(first)); // return a JoinHandle<Result<(), anyhow::Error>>
    let handle2 = tokio::spawn(echo(second));

    let (outcome1, outcome2) = tokio::join!(handle1, handle2); // wait for both handles to complete
    outcome1??;
    outcome2??;
    Ok(())
}

// echo server that accepts TCP connections and echoes the received data back to the client.
